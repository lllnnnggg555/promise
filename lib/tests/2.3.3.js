...
function testPromiseResolution(xFactory, test, stringRepresentation) {
    specify("via return from a fulfilled promise", function (done) {      
        if (stringRepresentation === 'an already-fulfilled promise for a synchronously-fulfilled custom thenable') {
            console.log(1212)
        }
...
function testCallingResolvePromise(yFactory, stringRepresentation, test) {
    describe("`y` is " + stringRepresentation, function () {
        describe("`then` calls `resolvePromise` synchronously", function () {
            ...
            testPromiseResolution(xFactory, test, stringRepresentation);
        });
